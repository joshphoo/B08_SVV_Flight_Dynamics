...
Vc     = (T1(:,5)-2)*0.514444; %calibrated airspeed in stationary flight conidition [m/s]
...
% Mach number
M1 = 1.0 + (gamma-1.0)/(2.0*gamma)*(rho0*p0*Vc.^2.0)
M2 = (1.0 + (gamma-1.0)/(2.0*gamma)*(rho0*p0*Vc.^2.0)).^(gamma/(gamma-1.0));
M3 = sqrt(2.0/(gamma-1.0)*((1.0+A.*((1.0 + (gamma-1.0)/(2.0*gamma)*(rho0*p0*Vc.^2.0)).^(gamma/(gamma-1.0))-1.0)).^((gamma-1.0)/gamma)-1.0));
