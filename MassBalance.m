...
Fweight = 4050 - Fused; %lbs
fuelmom = [];
for i = 1:length(Fweight)
    for j = 1:46
        if (Fweight(i) >=fuelmoments(j,1) ) && (Fweight(i) <=(fuelmoments(j,1)+100))
            fuelmom = ((Fweight-fuelmoments(j,1))*((fuelmoments((j+1),2)-fuelmoments(j,2))/100)+fuelmoments(j,2))*0.0254*0.45359237*100;
        end
    end
end 
xcg = [];
for i = 1:length(Fweight)
    Weight = OEW + sum(weights) + Fweight.*0.45359237; %kg
    xcg = [xcg,-(((xcgdatummom + fuelmom(i))/Weight(i))-6.643624)]; %m, - teken betekent ... meter voor de MAC
end
